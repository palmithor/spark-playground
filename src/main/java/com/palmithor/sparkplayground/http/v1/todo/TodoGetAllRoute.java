...
public class TodoGetAllRoute extends ApiRoute<EmptyRequest, ListResponse<TodoDTO>> {
    ...
    @Override
    protected ListResponse<TodoDTO> processImpl(final EmptyRequest payload,final Map<String, String> pathParams, final Map<String, String[]> queryParams) {
        TodoFilter filter = TodoFilter.ALL;
        if (queryParams.containsKey(RouteParams.QueryParams.FILTER_BY)) {
            TodoFilter filterFromQuery = TodoFilter.convertFromString(queryParams.get(RouteParams.QueryParams.FILTER_BY)[0].toUpperCase());
