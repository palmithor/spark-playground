...
public class TodoPutRouteFT {
    ...
    private TodoDTO dtoToUpdate;

    @Before
    public void setUp() {
        injector.injectMembers(this);
        injector.getInstance(MigrationUtils.class).startFresh();
        dtoToUpdate = todoDao.create(TodoDTO.create().withTitle("title").build());
    }

    @Test
    public void testUpdateTodo() throws Exception {
        SparkClient sparkClient = testServer.getClient();
        TodoRequest request = new TodoRequest("new title");
        PutMethod putMethod = sparkClient.put(
                V1Utils.preparePath("todos", String.valueOf(dtoToUpdate.getId())),
                gson.toJson(request));
        // TODO add auth
        HttpResponse httpResponse = sparkClient.execute(putMethod);
    ...
    @Test
    public void testUpdateNonExistingTodo() throws Exception {
        SparkClient sparkClient = testServer.getClient();
